...
export function init3DBackground() {
  ...
  function isDashboardActive() {
    ...
    // Если дашборд НЕ имеет класса hidden, значит он активен
    return dashboard && !dashboard.classList.contains('hidden');
  }

  function onTouchStart(event) {
    // ВАЖНОЕ ИСПРАВЛЕНИЕ:
    // Если мы в дашборде, разрешаем клик проходить (не вызываем preventDefault)
  ...
  function onTouchMove(event) {
    // Если мы в дашборде, разрешаем скролл и клики
    if (isDashboardActive()) return;
  ...
  function onTouchEnd(event) {
    // Если мы в дашборде, разрешаем клик
    if (isDashboardActive()) return;
  ...
  function onPointerMove(event) {
    ...
    const isInputHovered = document.body.classList.contains('hide-cursor-3d');

    // Если наведен - целевой радиус 0, иначе - стандартный
    const targetRadius = isInputHovered ? 0.0 : settings.cursorRadiusMin;

    // Плавная анимация изменения размера (Lerp)
    // Мы используем текущее значение uniform и плавно двигаем его к цели
    const currentRadius = material.uniforms.uCursorRadius.value;
    const newRadius = currentRadius + (targetRadius - currentRadius) * 0.1; // 0.1 - скорость исчезновения
    ...
    updateStory(cursorSphere3D.x, cursorSphere3D.y, newRadius, activeMerges, fps);
  }
  ...
  function render() {
    ...
    const isInputHovered = document.body.classList.contains('hide-cursor-3d');
    const targetRadius = isInputHovered ? 0.0 : settings.cursorRadiusMin;
    // Плавно уменьшаем радиус
    material.uniforms.uCursorRadius.value += (targetRadius - material.uniforms.uCursorRadius.value) * 0.1;
